#!/usr/bin/env python3
"""Fail closed on all preregistered held-out burst-dropout map pairs."""
...
from gen_static_burst_dropout_confirmation import (
    CONFIG_DIR,
    MANIFEST_PATH,
    PCD_DIR,
)
...
EXPECTED_VARIANTS = {
    "c1_mirror": {"shc1_mirror_clear", "shc1_mirror_hazard"},
    "c2_wide_offset": {
        "shc2_wide_offset_clear", "shc2_wide_offset_hazard"
    },
    "c3_near_short": {"shc3_near_short_clear", "shc3_near_short_hazard"},
}
...
def validate_variant(manifest: dict, variant: dict) -> dict:
    ...
    return {
        ...
        "inflated_bypass_segment_lengths_m": [round(value, 6)
...
def validate(manifest_path: Path = MANIFEST_PATH) -> dict:
    ...
    global_errors = []
    if manifest.get("schema") != "static-burst-dropout-confirmation-v1":
    ...
    if set(variants) != set(EXPECTED_VARIANTS):
        global_errors.append("unexpected held-out variant set")
    results = {
    ...
    return {
        "schema": "static-burst-dropout-structure-gate-v1",
        "status": "PASS" if not errors else "FAIL",
        "variants": results,
        "errors": errors,
    }
