#!/usr/bin/env python3
"""Audit actual-raycast fixed-Sector liveness for static blind doorway c1."""
...
EXPECTED = {
    "outgoing_x20": (20.0, 26.0, 1.5),
    "outgoing_x12": (12.0, 26.0, 1.5),
    "outgoing_x4": (4.0, 26.0, 1.5),
}


def analyze(paths, minimum_points_per_frame=50.0):
    errors = []
    metrics = {}
    ...
    for path in paths:
        ...
        name = next((station for station, expected in EXPECTED.items()
                     if len(position) == 3 and all(
                         math.isclose(a, b, abs_tol=1e-6)
                         for a, b in zip(position, expected))), None)
        if name is None:
        ...
        if not math.isclose(float(data.get("replay_yaw_deg", math.nan)),
                            180.0, abs_tol=1e-6):
            errors.append(f"{name}: yaw contract mismatch")
        if data.get("replay_velocity_xyz_mps") != [-7.0, 0.0, 0.0]:
    ...
    return {
        "schema": "static-blind-doorway-liveness-gate-v1",
