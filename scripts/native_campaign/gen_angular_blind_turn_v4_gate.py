#!/usr/bin/env python3
"""Generate the v4 observed-exit angular blind-turn development fixture.

The v3 flight isolated a fixed-Sector liveness confound: after the target was
passed, no geometry remained in the forward crop and SUPER correctly stopped
on stale map age.  V4 adds one distant northern observation wall only.  The
hazard, lower occluder, route and planner configuration remain unchanged.
"""
...
# This wall is 2.75 m from the preregistered y=26.1 bypass after accounting
# for its 0.15 m half-thickness.  Extending west beyond the final waypoint
# leaves points inside a 45-degree west-facing cone through mission finish.
OBSERVATION_WALL_START = (-10.0, 29.0)
OBSERVATION_WALL_END = (24.0, 29.0)
...
LIVENESS_STATIONS = (
    {"name": "outgoing_x20", "position_xyz_m": [20.0, 26.3, 1.2]},
    {"name": "outgoing_x12", "position_xyz_m": [12.0, 26.3, 1.2]},
    {"name": "outgoing_x4", "position_xyz_m": [4.0, 26.3, 1.2]},
)
...
def walls() -> tuple[Wall, ...]:
    ...
    return (
        ...
        Wall("north_observation", OBSERVATION_WALL_START, OBSERVATION_WALL_END),
    )
...
def generate() -> dict:
    ...
    wall_geometry = walls()
    point_count, wall_point_count = write_pcd(pcd_path, cylinders, wall_geometry)
    write_config(config_path, pcd_path.name)
    manifest = {
        "schema": "angular-blind-turn-v4-observed-exit-gate-v1",
        "purpose": "development gate only; do not pool with v1/v2/v3/evaluation",
        "map": MAP_NAME,
        "parent_geometry": "abt3_gate_open",
        "change_from_parent": "add north observation wall only",
        ...
        "observation_wall": {
            "start_xy_m": list(OBSERVATION_WALL_START),
            "end_xy_m": list(OBSERVATION_WALL_END),
            "thickness_m": WALL_THICKNESS_M,
            "height_m": OBSTACLE_HEIGHT_M,
            "role": "non-colliding forward-sector liveness support",
        },
        "guide_posts": [
        ...
        "route_check": {
            ...
            "flight_z_m": 1.50,
            "bounds_xy_m": [-1.0, 27.0, 20.0, 28.5],
        ...
        "liveness_check": {
            ...
            "yaw_deg": 180.0,
            "velocity_xyz_mps": [-7.0, 0.0, 0.0],
            "sector_half_angle_deg": 45.0,
            "sensing_horizon_m": 15.0,
            "minimum_filtered_points_per_frame": 50.0,
        },
