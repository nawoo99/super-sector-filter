#!/usr/bin/env python3
"""Generate shm1_h8 by changing only h7 common LiDAR rate from 5 to 4 Hz."""
...
import gen_static_heading_mismatch_h7 as h7
from gen_static_blind_corner_supplement import sha256


MANIFEST_PATH = h7.SCRIPT_DIR / "static_heading_mismatch_h8_manifest.json"
MAP_CLEAR = "shm1_h8_clear"
MAP_HAZARD = "shm1_h8_hazard"
SENSING_RATE_HZ = 4
...
def generate() -> dict:
    ...
    for item, target_name in zip(base["maps"], (MAP_CLEAR, MAP_HAZARD)):
        ...
        source_pcd = h7.PCD_DIR / f"{source_name}.pcd"
        target_pcd = h7.PCD_DIR / f"{target_name}.pcd"
        source_config = h7.CONFIG_DIR / f"{source_name}.yaml"
        target_config = h7.CONFIG_DIR / f"{target_name}.yaml"
    ...
    manifest.update({
        "schema": "static-heading-mismatch-v8",
        "role": "exploratory common 4 Hz static blind-fork threshold stress",
        "predecessor": "shm1_h7",
        "single_changed_factor": {
            "name": "common_lidar_sensing_rate_hz", "before": 5, "after": 4
        },
        "maps": maps,
