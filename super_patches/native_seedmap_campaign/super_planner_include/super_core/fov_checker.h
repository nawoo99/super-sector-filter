...
namespace super_planner {
    ...
    class FOVChecker {
        Mat3Df four_pts_;
        Mat3Df sixteen_pts_;
        // horizontal FoV half-angle in rad (<=0 disables the horizontal cut)
        double horizontal_angle_{-1.0};
        // true = center the horizontal cut on body heading (sector); false = on guide dir (adaptive)
        bool center_on_heading_{false};
    ...
    public:
        ...
        void setHorizontalAngleDeg(const double &deg) {
            horizontal_angle_ = (deg > 0) ? deg / 180.0 * M_PI : -1.0;
        }
        void setCenterOnHeading(const bool &b) { center_on_heading_ = b; }
        ...
        bool cutPolyByFov(const Vec3f & robot_p, const super_utils::Quatf & robot_q,
            ...
            // center the FoV on body heading (sector) or on the guide direction (adaptive)
            rotation_matrix3 = Eigen::AngleAxisd(center_on_heading_ ? 0.0 : yaw_in_body_frame,
        ...
        void getFovCheckPlane(
            ...
            // horizontal +-angle cut: two side planes containing the sensor vertical axis,
            // at +-horizontal_angle_ from the forward (R*x) direction. Interior (forward) satisfies n.x+d<=0.
            if (horizontal_angle_ > 0) {
                const double h = horizontal_angle_;
                // body-frame inward normals for |yaw| <= h (forward = +x): derived so +x is interior
                Eigen::Vector3d nb_left(-sin(h), cos(h), 0.0);
                Eigen::Vector3d nb_right(-sin(h), -cos(h), 0.0);
                Eigen::Vector3d NL = R * nb_left;
                Eigen::Vector3d NR = R * nb_right;
                Eigen::MatrixX4d ext(fov_planes.rows() + 2, 4);
                ext.topRows(fov_planes.rows()) = fov_planes;
                ext.row(fov_planes.rows())     << NL(0), NL(1), NL(2), -NL.dot(t);
                ext.row(fov_planes.rows() + 1) << NR(0), NR(1), NR(2), -NR.dot(t);
                fov_planes = ext;
            }
        }
