...
namespace rog_map {
    ...
    public:
        ...
        // Immutable planner snapshots only need the cells whose inflated
        // occupancy bit crossed zero since the previous publication.  Hash
        // indices are stable for the fixed-map profile and avoid copying the
        // complete (tens of millions of cells) inflation buffer every scan.
        std::vector<int> consumeSnapshotDirtyHashes();

        bool isOccupiedInflateHash(const int hash_id) const {
            return imd_.occ_inflate_cnt[hash_id] > 0;
        }
    ...
    private:
        ...
        int inf_num_{0};
        double inf_t_{0.0};
        std::unordered_set<int> snapshot_dirty_hashes_;
