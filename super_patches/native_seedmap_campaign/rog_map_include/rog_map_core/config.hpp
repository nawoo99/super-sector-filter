...
#define CMAKE_ROOT_DIR(name) (string(string(ROOT_DIR) + name))
    ...
    public:
        ...
        Config(const string &cfg_path, const string &name_space = "rog_map") {
            ...
            // Only used when raycasting/enable is false. Marks cells along
            // the sensor-to-hit ray as observed-free (see raycastProcess's
            // !raycasting_en branch), without the full probabilistic
            // hit/miss accumulation raycasting/enable=true does. Kept
            // separate from raycast_range_max/ray_range so enabling this
            // doesn't scale per-scan marching cost with the full sensor
            // range -- only near-field clearance matters for the guard
            // check this feeds (e.g. emergency-brake stopping distance).
            loader.LoadParam(name_space + "/raycasting/observed_mark_range_max",
                             observed_mark_range_max, 10.0);
...
#define logit(x) (log((x) / (1 - (x))))
        ...
        double observed_mark_range_max{10.0};
