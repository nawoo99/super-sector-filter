...
#define CONFIG_FILE_DIR(name) (std::string(std::string(ROOT_DIR) + "config/"+(name)))
    ...
    // Planner main/replan callbacks may both be CPU-heavy. Reserve independent
    // executor capacity for map commits and command publication instead of
    // relying on a host-dependent default thread count.
