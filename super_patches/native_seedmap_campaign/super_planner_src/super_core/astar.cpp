...
namespace path_search {
    ...
    Astar::setup(const Vec3f &start_pt, const Vec3f &goal_pt, const int &flag, const double &searching_horizon) {
        ...
        if (md_.use_inf_neighbor) {
            const double base_clearance = md_.use_inf_map
                    ? inflated_map_clearance_m_ : 0.0;
            const double extra_clearance = std::max(
                    0.0, hard_clearance_m_ - base_clearance);
            const int neighbor_step = static_cast<int>(std::ceil(
                    extra_clearance / md_.resolution));
            setFineInfNeighbors(neighbor_step);
        }
    ...
    void Astar::setClearanceRadii(const double hard_clearance_m,
                                  const double inflated_map_clearance_m) {
        hard_clearance_m_ = std::max(0.0, hard_clearance_m);
        inflated_map_clearance_m_ = std::max(0.0, inflated_map_clearance_m);
    }
                ...
                for (int dy = -1; dy <= 1; dy++)
                    for (int dz = -1; dz <= 1; dz++) {
                        ...
                        const std::size_t avoidance_count = std::min(
                                ...
                                avoidance_radii.size());
                        for (std::size_t i = 0; i < avoidance_count; ++i) {
                            const double radius = avoidance_radii[i];
                            if (!std::isfinite(radius) || radius <= 0.0 ||
                                !avoidance_centers[i].array().isFinite().all()) {
                                continue;
                            }
                            ...
                            if (neighbor_distance < radius &&
                                (current_distance >= radius ||
                                 neighbor_distance <= current_distance +
                                         0.25 * md_.resolution)) {
                                topology_excluded = true;
                                break;
                            }
                        }
                        if (topology_excluded) {
                            continue;
                        }
                        ...
                        if (md_.use_inf_map) {
                            ...
                            if (md_.use_inf_neighbor &&
                                neighbor_type != OCCUPIED &&
                                neighborHaveOne(OCCUPIED, neighborIdx)) {
                                neighbor_type = OCCUPIED;
                            }
                        } else {
                            if (!md_.use_inf_neighbor) {
