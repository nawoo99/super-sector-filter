...
//            cout << "bd  * bh: " << (bd * bh).transpose().maxCoeff() << endl;
        ...
        // Preferred-margin corridors: pick ONE uniform margin for this whole segment,
        // as large as possible up to pref_margin_ but never more than what the tightest
        // obstacle point along the seed line actually allows (and never below robot_r_,
        // which the FAILED check below already guarantees is feasible). This keeps a
        // single corridor's planes internally consistent instead of letting some walls
        // sit at pref_margin_ and neighboring walls fall back to robot_r_ independently,
        // which produced lopsided corridors and measurably hurt tight-map performance.
        double local_margin = robot_r_;
        if (pref_margin_ > robot_r_ + epsilon_) {
            double d_min_seed = INFINITY;
            for (int i = 0; i < N; ++i) {
                const double d = distancePointToSegment(pc.col(i), a, b);
                if (d < d_min_seed) {
                    d_min_seed = d;
                }
            }
            local_margin = (N > 0) ? std::min(pref_margin_, std::max(robot_r_, d_min_seed - 1e-2))
                                    : pref_margin_;
        }
        const Ellipsoid local_sphere_template =
                (std::abs(local_margin - pref_margin_) < epsilon_)
                ? sphere_template_
                : Ellipsoid(Mat3f::Identity(), local_margin * Vec3f(1, 1, 1), Vec3f(0, 0, 0));
...
//        bool infeasible_problem{false};
        ...
        int dbg_num_obstacle_planes = 0;
        int dbg_num_fallback_planes = 0;

        for (int loop = 0; loop < iter_num_; ++loop) {
            dbg_num_obstacle_planes = 0;
            dbg_num_fallback_planes = 0;
...
//                        infeasible_problem = true;
                    ...
                    if (robot_r_ < epsilon_) {
                        ...
                        Ellipsoid E_pe(C_inv * local_sphere_template.C(), pt_e);
                        Vec3f close_pt_e;
...
//                std::cout << " -- [CIRI] hPoly: " << hPoly << std::endl;
        ...
        logMarginDebug(a, b, dbg_num_obstacle_planes, dbg_num_fallback_planes, local_margin);

        return SUCCESS;
    }

    void CIRI::logMarginDebug(const Vec3f& a, const Vec3f& b, int num_obstacle_planes, int num_fallback_planes, double local_margin) {
        if (pref_margin_ <= robot_r_ + epsilon_) {
            return;
        }
        static bool env_checked = false;
        static bool debug_en_local = false;
        if (!env_checked) {
            env_checked = true;
            debug_en_local = (std::getenv("CIRI_MARGIN_DEBUG") != nullptr);
        }
        if (!debug_en_local) {
            return;
        }
        ...
        if (!log_opened) {
            log_opened = true;
            dbg_log.open("/tmp/ciri_margin_debug.csv", std::ios::out | std::ios::app);
        }
        if (!dbg_log.is_open()) {
            return;
        }
        const bool is_point_seed = (a - b).norm() < 1e-6;
        const auto now = std::chrono::steady_clock::now().time_since_epoch();
        const double t_s = std::chrono::duration<double>(now).count();
        dbg_log << std::fixed << std::setprecision(4)
                << t_s << "," << a.x() << "," << a.y() << "," << a.z() << ","
                << (is_point_seed ? 1 : 0) << ","
                << num_obstacle_planes << "," << num_fallback_planes << ","
                << local_margin << endl;
        dbg_log.flush();
    }
    ...
    void CIRI::setupParams(double robot_r, int iter_num, double pref_margin) {
        robot_r_ = robot_r;
        pref_margin_ = (pref_margin > robot_r_) ? pref_margin : robot_r_;
        iter_num_ = iter_num;
        // Tangent planes are placed pref_margin_ away from the nearest obstacle point
        // (>= robot_r_), giving the corridor extra breathing room where space allows.
        // The hard FAILED feasibility check and the seed-line-cut fallback below both
        // stay keyed to the true robot_r_, so this cannot make a feasible line infeasible.
        sphere_template_ = Ellipsoid(Mat3f::Identity(), pref_margin_ * Vec3f(1, 1, 1), Vec3f(0, 0, 0));
