...
Polytope::Polytope(MatD4f _planes) {
    planes = _planes;
    face_obstacle_flags_.assign(planes.rows(), 0);
    undefined = false;
}
...
Polytope Polytope::CrossWith(const Polytope &b) const {
    ...
    Polytope out;
    out.SetPlanes(curIH);
    std::vector<uint8_t> flags = face_obstacle_flags_;
    const auto &other_flags = b.GetFaceObstacleFlags();
    flags.insert(flags.end(), other_flags.begin(), other_flags.end());
    out.SetFaceObstacleFlags(std::move(flags));
    return out;
}
...
const std::vector<uint8_t> &Polytope::GetFaceObstacleFlags() const {
    return face_obstacle_flags_;
}
...
void Polytope::SetFaceObstacleFlags(std::vector<uint8_t> flags) {
    if (flags.size() != static_cast<std::size_t>(planes.rows())) {
        face_obstacle_flags_.assign(planes.rows(), 0);
        return;
    }
    face_obstacle_flags_ = std::move(flags);
}
