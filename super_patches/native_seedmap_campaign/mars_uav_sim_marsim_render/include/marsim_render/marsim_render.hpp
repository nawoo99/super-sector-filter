...
((value) < (min) ? (min) : ((value) > (max) ? (max) : (value)))
    ...
    public:
        ...
        explicit MarsimRender(const std::string& cfg_path_) {
            ...
            // Pixel ray directions depend only on the configured angular
            // resolution.  Precompute them once: the old conversion evaluated
            // four trigonometric functions for every emitted LiDAR point in
            // every frame (roughly 115k points for GENERAL_360).
            ray_sin_azimuth_.resize(cfg_.width);
            ray_cos_azimuth_.resize(cfg_.width);
            ray_sin_elevation_.resize(cfg_.height);
            ray_cos_elevation_.resize(cfg_.height);
            const decimal_t angular_resolution =
                    cfg_.polar_resolution / 180.0 * M_PI;
            const decimal_t u0 = cfg_.width * 0.5;
            const decimal_t v0 = cfg_.height * 0.5;
            for (int u = 0; u < cfg_.width; ++u) {
                const decimal_t azimuth = angular_resolution * (u - u0);
                ray_sin_azimuth_[u] = std::sin(azimuth);
                ray_cos_azimuth_[u] = std::cos(azimuth);
            }
            for (int v = 0; v < cfg_.height; ++v) {
                const decimal_t elevation = angular_resolution * (v - v0);
                ray_sin_elevation_[v] = std::sin(elevation);
                ray_cos_elevation_[v] = std::cos(elevation);
            }
