...
namespace marsim {
    ...
    void MarsimRender::render_pointcloud(
        ...
        // render
        // ------
        // Defensive rebind: a GLFW/GL context is only current on whichever
        // thread last bound it, and glfwPollEvents() below additionally
        // requires the GLFW-creating/main thread. The caller must keep this
        // function pinned to that one thread (see PerfectDrone's dedicated
        // single-threaded render executor); this call is a cheap no-op
        // reaffirmation of that invariant, not a substitute for it.
        glfwMakeContextCurrent(window);
        if (!glfwGetCurrentContext()) {
